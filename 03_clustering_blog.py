...
plants_2023['carbon_intensity'] = plants_2023['Plant annual CO2 emissions (tons)'] / plants_2023['Plant annual net generation (MWh)']
plants_2023['capacity_factor'] = plants_2023['Plant annual net generation (MWh)'] / (plants_2023['Plant nameplate capacity (MW)'] * 8760)
features = ['log_generation', 'log_co2', 'carbon_intensity', 'capacity_factor', 'nox_intensity', 'so2_intensity']
...
for k in range(2, 11):
    ...
    results.append({'k': k, 'inertia': kmeans.inertia_, 'silhouette': silhouette_score(X_scaled, labels), 'calinski': calinski_harabasz_score(X_scaled, labels)})
...
cluster_profiles = plants_2023.groupby('cluster').agg({'log_generation': 'median', 'carbon_intensity': 'median', 'capacity_factor': 'median', 'Plant nameplate capacity (MW)': 'median'}).round(3)
logger.info('\nCluster Profiles:')
...
plt.subplot(1, 2, 2)
plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c=cluster_labels[sample_idx], cmap='tab10', alpha=0.6, s=20)
plt.xlabel('t-SNE 1')
plt.ylabel('t-SNE 2')
...
def benchmark_plant(plant_row, cluster_data):
    ...
    return {'cluster_median': cluster_median, 'plant_value': plant_value, 'percentile': percentile, 'vs_median': (plant_value - cluster_median) / cluster_median * 100}
coal_cluster_id = 0
...
state_profiles = plants_2023.groupby('Plant state abbreviation').apply(lambda df: pd.Series({'pct_cluster_0': (df['cluster'] == 0).sum() / len(df) * 100, 'pct_cluster_1': (df['cluster'] == 1).sum() / len(df) * 100, 'pct_cluster_2': (df['cluster'] == 2).sum() / len(df) * 100, 'pct_cluster_3': (df['cluster'] == 3).sum() / len(df) * 100, 'pct_cluster_4': (df['cluster'] == 4).sum() / len(df) * 100}))
...
for i, (idx, dist) in enumerate(zip(similar_idx, similar_dist), 1):
    ...
    logger.info(f"{i}. {plant.get('Plant name', 'Unknown')} ({plant.get('Plant state abbreviation', '??')}) - Distance: {dist:.3f}")
kmeans = KMeans(n_clusters=k, random_state=42, n_init=10)
labels = kmeans.fit_predict(X_scaled)
results.append({'k': k, 'inertia': kmeans.inertia_, 'silhouette': silhouette_score(X_scaled, labels), 'calinski': calinski_harabasz_score(X_scaled, labels)})
logger.info(f"K={k}: Silhouette={results[-1]['silhouette']:.3f}")
...
percentile = (cluster_data['carbon_intensity'] < plant_value).mean() * 100
return {'cluster_median': cluster_median, 'plant_value': plant_value, 'percentile': percentile, 'vs_median': (plant_value - cluster_median) / cluster_median * 100}
lambda df: pd.Series({'pct_cluster_0': (df['cluster'] == 0).sum() / len(df) * 100, 'pct_cluster_1': (df['cluster'] == 1).sum() / len(df) * 100, 'pct_cluster_2': (df['cluster'] == 2).sum() / len(df) * 100, 'pct_cluster_3': (df['cluster'] == 3).sum() / len(df) * 100, 'pct_cluster_4': (df['cluster'] == 4).sum() / len(df) * 100})
states_in_cluster = state_profiles[state_profiles['state_cluster'] == i].index.tolist()
logger.info(f"\nCluster {i}: {', '.join(states_in_cluster)}")
logger.info(state_profiles[state_profiles['state_cluster'] == i].mean())
nn = NearestNeighbors(n_neighbors=n_neighbors + 1)
